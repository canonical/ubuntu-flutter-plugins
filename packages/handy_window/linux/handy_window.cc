...
gboolean handy_window_is_closable(GtkWindow* window) {
  GtkWidget* header_bar = handy_window_get_header_bar(GTK_WINDOW(window));
#ifdef HAVE_LIBHANDY
  if (HDY_IS_HEADER_BAR(header_bar)) {
...
void handy_window_set_closable(GtkWindow* window, gboolean closable) {
  GtkWidget* header_bar = handy_window_get_header_bar(window);
  g_return_if_fail(header_bar != nullptr);
#ifdef HAVE_LIBHANDY
  if (HDY_IS_HEADER_BAR(header_bar)) {
